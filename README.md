...
137 tools: 60 read-only and 77 write. Read-only tools are always available. On the hosted server the write tools are registered too and gated by your credential's scopes; the local binary registers them only when started with `--allow-writes`.
...
**Conventions:** destructive tools (`delete_*`, `uninstall_extension`, `cancel_cluster_upgrade`, `update_theme_content`, `update_theme_settings`, `restart_cluster_instances`) require `confirm=true`. `update_theme_settings` turns `exact_theme_names` on or off for the workspace; the caller's API key must have been minted for a workspace owner or admin, or it gets `403` even with `themes:write`. Turning it on moves existing themes to their exact served names in the background; a theme whose content was replaced under its exact name reports `restart_required: true` from `get_theme`/`list_themes`/`update_theme_content` until `restart_cluster_instances` rolls that cluster's Keycloak instances. A restart may be deferred to the cluster's maintenance window instead of applying immediately, reported as `deferred: true` and, when known, `next_window`. `create_cluster` is asynchronous: poll `get_cluster` until the cluster is `available`. `create_domain` returns the DNS records the customer must create; `verify_domain` triggers a DNS check. `set_theme_assignment` activates a custom theme per Keycloak theme type (empty string resets to the built-in default). `update_theme_content` replaces a theme's archive in place (base64 ZIP or Keycloakify JAR in `content_base64`), keeping the theme's ID, name and realm and application assignments, so editing a theme no longer means deleting and re-uploading it; it needs `confirm=true` because the archive it overwrites is not recoverable, and `update_theme` still changes only the name, description and version. See [docs/theme-content-update.md](docs/theme-content-update.md) for how that call is made. `update_cluster_security` leaves CAPTCHA settings untouched. Realm import/export moves one realm's configuration and is separate from `create_export`, which dumps a whole cluster's database: both are asynchronous, and the realm archive is always encrypted, so the password used to export it is needed to import it again. A realm can be imported straight from an existing export (`source_export_id`) or from an uploaded archive (`create_realm_import_upload_url`, PUT, then `upload_s3_key`); importing creates a realm and refuses a name collision rather than overwriting, and needs `confirm=true` because it brings users and credentials with it.

**Cluster locations:** `location` on `create_cluster`, and in what `list_clusters`, `get_cluster` and `list_cluster_locations` return, is one of `us` (US East), `us-west` (US West), `ca` (Canada), `eu` (Europe) or `au` (Australia), in any case. `us` always means US East. `us-west` is only open to workspaces that have been granted access to it: `list_cluster_locations` lists every location with `available` set for your workspace, and creating a cluster in a location you cannot use returns the API's `400` "location not available".
