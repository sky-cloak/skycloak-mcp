...
**Read-only** (always available): `list_clusters`, `get_cluster`, `list_realms`, `list_applications`, `list_identity_providers`, `get_logs`, `get_security_logs`, `query_events`, `list_domains`, `get_domain`, `list_themes`, `get_theme_assignment`, `get_login_branding`, `get_email_branding`, `list_extensions`, `list_cluster_extensions`, `list_exports`, `get_export`, `list_realm_roles`, `list_realm_groups`, `list_realm_users`, `get_realm_user`.
