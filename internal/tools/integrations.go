...
func registerSIEMReadTools(s *mcp.Server, api API) {
	mcp.AddTool(s, &mcp.Tool{Name: "skycloak_list_siem_destinations", Description: "List SIEM destinations configured for the workspace.", Annotations: &mcp.ToolAnnotations{OpenWorldHint: ptr(false), ReadOnlyHint: true, Title: "List SIEM destinations"}}, listSIEMDestinationsHandler(api))
	mcp.AddTool(s, &mcp.Tool{Name: "skycloak_get_siem_destination", Description: "Get a SIEM destination by ID.", Annotations: &mcp.ToolAnnotations{OpenWorldHint: ptr(false), ReadOnlyHint: true, Title: "Get SIEM destination"}}, getSIEMDestinationHandler(api))
}

func registerSIEMWriteTools(s *mcp.Server, api API) {
	mcp.AddTool(s, &mcp.Tool{Name: "skycloak_create_siem_destination", Description: "Create a SIEM destination. Credentials are write-only and are not returned.", Annotations: &mcp.ToolAnnotations{OpenWorldHint: ptr(false), ReadOnlyHint: false, DestructiveHint: ptr(false), Title: "Create SIEM destination"}}, createSIEMDestinationHandler(api))
	...
	mcp.AddTool(s, &mcp.Tool{Name: "skycloak_delete_siem_destination", Description: "Delete a SIEM destination. Set confirm=true to proceed.", Annotations: &mcp.ToolAnnotations{OpenWorldHint: ptr(false), ReadOnlyHint: false, DestructiveHint: ptr(true), Title: "Delete SIEM destination"}}, deleteSIEMDestinationHandler(api))
	mcp.AddTool(s, &mcp.Tool{Name: "skycloak_test_siem_destination", Description: "Send a test event to a SIEM destination.", Annotations: &mcp.ToolAnnotations{OpenWorldHint: ptr(true), ReadOnlyHint: false, DestructiveHint: ptr(false), Title: "Test SIEM destination"}}, testSIEMDestinationHandler(api))
}

func registerWebhookReadTools(s *mcp.Server, api API) {
	mcp.AddTool(s, &mcp.Tool{Name: "skycloak_list_webhook_event_types", Description: "List webhook event types. Optionally filter by source: platform or keycloak.", Annotations: &mcp.ToolAnnotations{OpenWorldHint: ptr(false), ReadOnlyHint: true, Title: "List webhook event types"}}, listWebhookEventTypesHandler(api))
	mcp.AddTool(s, &mcp.Tool{Name: "skycloak_list_webhook_subscriptions", Description: "List webhook subscriptions. Optionally filter by source, cluster_id, and enabled.", Annotations: &mcp.ToolAnnotations{OpenWorldHint: ptr(false), ReadOnlyHint: true, Title: "List webhook subscriptions"}}, listWebhookSubscriptionsHandler(api))
	mcp.AddTool(s, &mcp.Tool{Name: "skycloak_get_webhook_subscription", Description: "Get a webhook subscription by ID.", Annotations: &mcp.ToolAnnotations{OpenWorldHint: ptr(false), ReadOnlyHint: true, Title: "Get webhook subscription"}}, getWebhookSubscriptionHandler(api))
}

func registerWebhookWriteTools(s *mcp.Server, api API) {
	mcp.AddTool(s, &mcp.Tool{Name: "skycloak_create_webhook_subscription", Description: "Create a webhook subscription. Signing secrets and authorization headers are write-only.", Annotations: &mcp.ToolAnnotations{OpenWorldHint: ptr(false), ReadOnlyHint: false, DestructiveHint: ptr(false), Title: "Create webhook subscription"}}, createWebhookSubscriptionHandler(api))
	...
	mcp.AddTool(s, &mcp.Tool{Name: "skycloak_delete_webhook_subscription", Description: "Delete a webhook subscription. Set confirm=true to proceed.", Annotations: &mcp.ToolAnnotations{OpenWorldHint: ptr(false), ReadOnlyHint: false, DestructiveHint: ptr(true), Title: "Delete webhook subscription"}}, deleteWebhookSubscriptionHandler(api))
	mcp.AddTool(s, &mcp.Tool{Name: "skycloak_test_webhook_subscription", Description: "Send a test event to a webhook subscription.", Annotations: &mcp.ToolAnnotations{OpenWorldHint: ptr(true), ReadOnlyHint: false, DestructiveHint: ptr(false), Title: "Test webhook subscription"}}, testWebhookSubscriptionHandler(api))
}
