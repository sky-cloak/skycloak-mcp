...
func registerBrandingReadTools(s *mcp.Server, api API) {
	addTool(s, &mcp.Tool{
		...
		Description: "List the custom themes uploaded to a cluster, with their IDs, status, and theme types. restart_required is true on a theme whose replaced content is not live yet; see skycloak_get_theme_settings and skycloak_restart_cluster_instances.",
	...
	addTool(s, &mcp.Tool{
		Name: "skycloak_get_theme_settings",
		Description: "Get the workspace's theme naming policy: whether exact_theme_names is on. " +
			"When on, every theme in the workspace is served from a folder named exactly like the theme, and that name never changes across content replaces. " +
			"When off (the default), the served folder changes on each content replace, unless the theme was created by a platform migration.",
		Annotations: &mcp.ToolAnnotations{OpenWorldHint: ptr(false), ReadOnlyHint: true, Title: "Get theme settings"},
	}, getThemeSettingsHandler(api))
}
...
func registerBrandingWriteTools(s *mcp.Server, api API) {
	...
	addTool(s, &mcp.Tool{
		Name: "skycloak_update_theme_settings",
...
// restartRequiredSuffix flags a theme whose replaced content is not live yet,
// so the text output carries the same signal as the restart_required field.
func restartRequiredSuffix(restartRequired bool) string {
	if restartRequired {
...
func themeSettingsText(s *skycloak.ThemeSettings) string {
	if s.ExactThemeNames {
		return "exact_theme_names is on: themes are served from folders named exactly like the theme."
	}
	return "exact_theme_names is off: the served folder changes on each content replace."
}

func getThemeSettingsHandler(api API) mcp.ToolHandlerFor[NoInput, skycloak.ThemeSettings] {
	return func(ctx context.Context, _ *mcp.CallToolRequest, _ NoInput) (*mcp.CallToolResult, skycloak.ThemeSettings, error) {
		s, err := api.GetThemeSettings(ctx)
		if err != nil {
			return toolError(err), skycloak.ThemeSettings{}, nil
		}
		return okResult(themeSettingsText(s)), *s, nil
	}
}
...
func updateThemeSettingsHandler(api API) mcp.ToolHandlerFor[UpdateThemeSettingsInput, skycloak.ThemeSettings] {
	return func(ctx context.Context, _ *mcp.CallToolRequest, in UpdateThemeSettingsInput) (*mcp.CallToolResult, skycloak.ThemeSettings, error) {
		...
		text := "Updated theme settings. " + themeSettingsText(s)
		if s.ExactThemeNames {
			text += " Existing themes are moving to their exact names in the background; a theme with restart_required=true needs skycloak_restart_cluster_instances before the new content is live."
		}
		return okResult(text), *s, nil
	}
}
